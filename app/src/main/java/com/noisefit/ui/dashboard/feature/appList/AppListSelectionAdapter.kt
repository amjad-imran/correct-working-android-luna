...
class AppListSelectionAdapter(private val action: AppListSelectionListener) :
    RecyclerView.Adapter<AppListSelectionAdapter.ViewHolder>(), ItemMoveCallbackListener.Listener {
    ...
    inner class ViewHolder(val binding: RowSportSelectionBinding) :
        ...
        fun bind(widget: Widget) {
            ...
            binding.imageView8.setImageResource(ImageUtil().getAppIconImageMighty(widget.functionId))
            if (mEditMode) {
                if (widget.sortable) {
...
interface AppListSelectionListener {
