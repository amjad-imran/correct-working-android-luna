...
@AndroidEntryPoint
class BodyTemperatureGraphFragment :
    BaseFragment<FragmentBodyTemperatureGraphBinding>(FragmentBodyTemperatureGraphBinding::inflate),
    OnChartValueSelectedListener {

    private val viewModel: BodyTemperatureGraphViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            viewModel.steps = it.getFloat(ARG_PARAM5, 0f)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        StepsChartUtils.setChart(
            resources.getColor(R.color.body_temp),
            binding.barChart,
            graphInterval,
            0,
    ...
    private fun initUi(graphInterval: GraphInterval) {
        ...
        binding.lytGraphDetailsHeader.apply {
            if(viewModel.steps == 0f){
                tvAverageSteps.text = "_"
                tvTotalStepsCount.text = "_"
            }else{
                val tempWithUnit = viewModel.getBodyTempWithUnit(viewModel.steps)
                tvAverageSteps.text = tempWithUnit
    ...
    private fun inflateView(marginStart: Float, e: Entry) {
        ...
        if (viewModel.markEntryList.isEmpty()) {
            return
        }
        removeMarkerView()
...
//        LOGS.d("asdassaddsa ${totalMargin} ${markerView!!.measuredWidth} ${binding.barChart.width}")
    ...
    companion object {
        @JvmStatic
        fun newInstance(
            bodyTempDataList: List<BodyTempHistory>,
            historyType: String,
            range: String,
            date: String,
            steps: Float
        ) =
            BodyTemperatureGraphFragment().apply {
                arguments = Bundle().apply {
                    putParcelableArrayList(ARG_PARAM1, bodyTempDataList as ArrayList)
                    putString(ARG_PARAM2, historyType)
                    putString(ARG_PARAM4, date)
                    putFloat(ARG_PARAM5, steps)
