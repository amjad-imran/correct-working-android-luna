...
const val QR_UPLOAD_NAME = "QR_UPLOAD_NAME"

@AndroidEntryPoint
class BottomSheetAddName :
    BaseBottomSheetWithTransparent<BottomSheetQrAddNameBinding>(BottomSheetQrAddNameBinding::inflate) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        qrCodeObj?.title?.let {//Edit mode case
    ...
    override fun initListener() {
        ...
        binding.btnBack.setOnClickListener {
            navigateUpSafe()
            setFragmentResult(
                QR_UPLOAD_NAME, bundleOf("isBackPressed" to true)
            )
        }
        binding.btnNext.setOnClickListener {
            val enteredText = binding.etName.text.toString().trim()

            if (enteredText.isEmpty()) {
                context.showShortToast("Add name to continue")
                return@setOnClickListener
            }
            qrCodeObj?.title = enteredText

            setFragmentResult(
                QR_UPLOAD_NAME, bundleOf("isBackPressed" to false, "qrCodeObj" to qrCodeObj)
            )
            navigateUpSafe()
        }
    }
