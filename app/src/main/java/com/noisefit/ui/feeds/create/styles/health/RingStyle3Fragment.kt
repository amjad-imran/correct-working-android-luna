...
import com.noisefit.luna.databinding.FragmentRingStyle3Binding
...
@AndroidEntryPoint
class RingStyle3Fragment :
    BaseFragment<FragmentRingStyle3Binding>(FragmentRingStyle3Binding::inflate) {
    ...
    private fun setRing() {
        binding.apply {
            ...
            dynamicArcView.addSeries(
                ApplicationUtils.seriesItemWithInset(
                    requireActivity(), 100f, 100f, R.color.calories_arc_bg, 80f,16f
                )
            )
            dynamicArcView.addSeries(
                ApplicationUtils.seriesItemWithoutInset(
                    requireActivity(), 100f, 100f, R.color.steps_arc_bg, 16f
                )
            )
        }
        ...
        binding.apply {
            ...
            if (viewModel.caloriesProgressCompleted != caloriesValue) {
                ...
                val caloriesIndex: Int = dynamicArcView.addSeries(
                    ApplicationUtils.seriesItemWithInset(
                        requireActivity(), 0f, 100f, R.color.calories_arc, 80f,16f
                    )
                )
                dynamicArcView.addEvent(
