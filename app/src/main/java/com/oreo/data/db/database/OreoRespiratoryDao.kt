...
@Dao
interface OreoRespiratoryDao : BaseDao<OreoRespiratoryData> {
    @Query("SELECT * FROM respiratory where date = :date")
    fun getTodayData(date: String): OreoRespiratoryData?


    @Query("UPDATE respiratory SET break_up = :breakUp,is_synced = :is_synced  WHERE date = :date")
    fun updateViaDate(breakUp: String, date: String, is_synced: Boolean)


    @Query("Delete FROM respiratory where date = :date")
    fun deleteTodayData(date: String)

    @Query("UPDATE respiratory SET is_google_fit_sync = :is_google_fit_sync WHERE id IN (:ids)")
    fun updateGoogleFitStatus(ids: List<Int>, is_google_fit_sync: Boolean)

    @Query("SELECT * FROM respiratory where is_synced = :is_synced")
    fun getServerUnSyncData(is_synced: Boolean): List<OreoRespiratoryData>?
