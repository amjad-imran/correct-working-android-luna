    ...
    private val fitnessDataCallBack: FitnessDataCallBack =
        object : FitnessDataCallBack {
            ...
            override fun onRingSleepNAP(p0: MutableList<RingSleepNapBean>?) {
                ...
                //val dummyNap = Gson().fromJson<List<RingSleepNapBean>>("[{\"existSleepNap\":true, \"asleepNapTime\":1736802000, \"wakeupNapTime\":1736803740, \"sleepNapDuration\":1740, \"date\":\"2025-01-14 00:00:00\"}]")
            ...
            override fun onRingStressDetectionData(p0: RingStressDetectionBean?) {
                ...
                AppLogs.sendAppLogs("$TRACK_TAG onRingStressDetectionData : $p0")
                watchDataStore.testSaveStressData(Gson().toJson(p0))
                if (p0 == null) return

                userActivityDataCallbacks?.onUserActivityDataReceived(
                    UserActivityCallback.OreoBodyStressDataObtained(
                        oreoDataConverter.parseBodyStressData(p0)
                    )
                )
            }
