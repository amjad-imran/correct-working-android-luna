...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
        ...
        for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
            ...
            if (showCandleBar) {
                ...
                if (open > close) { // decreasing
                    ...
                    float rad = (mBodyBuffers[2] - mBodyBuffers[0]) / 2;
                    RectF rectF = new RectF(mBodyBuffers[0], mBodyBuffers[3], mBodyBuffers[2], mBodyBuffers[1]);

                    // c.drawRoundRect(rectF,rad,rad,mRenderPaint);
                    c.drawRoundRect(
                            rectF, rad, rad,
                            mRenderPaint);
    ...
    canvas.drawRoundRect(left, top, right, bottom, rad, rad, mRenderPaint)
                    ...
                    RectF rectF = new RectF(mBodyBuffers[0], mBodyBuffers[1], mBodyBuffers[2], mBodyBuffers[3]);
                    float rad = (mBodyBuffers[2] - mBodyBuffers[0]) / 2;
                    c.drawRoundRect(rectF, rad, rad, mRenderPaint);
