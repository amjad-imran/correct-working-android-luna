...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    private ArrayList<Entry> plottedEntry = new ArrayList();
    /**
    ...
    private float lastGradientPoint = 0f;
    protected Path cubicPathFill = new Path();
    protected Path singlePointPath = new Path();
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            cubicPathFill.moveTo(cur.getX(), cur.getY() * phaseY);
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                plottedEntry.add(cur);
                final float cpx = (prev.getX())
                        + (cur.getX() - prev.getX()) / 2.0f;


//                Log.d("linechartData", cpx + " " + cur.getX() + " " + prev.getY() + " " + cur.getY());
                if (prev.getY() != 0f && cur.getY() == 0f) {
                   // cubicPathFill.moveTo(cur.getX(), cur.getY() * phaseY);
                    // cubicPath.drw
                    cubicPath.addCircle(prev.getX(), prev.getY(), .1f, Path.Direction.CW);
//                    Log.d("linechartData", "inside 1");

                } else if (prev.getY() == 0f && cur.getY() != 0f) {
                 //   cubicPathFill.moveTo(cur.getX(), cur.getY() * phaseY);
                    cubicPath.addCircle(cur.getX(), cur.getY(), .1f, Path.Direction.CCW);
//                    Log.d("linechartData", "inside 2");
                } else if (cur.getY() != 0f && prev.getY() != 0f) {
                    lastGradientPoint = cur.getX();
                    cubicPath.cubicTo(
                            cpx, prev.getY() * phaseY,
                            cpx, cur.getY() * phaseY,
                            cur.getX(), cur.getY() * phaseY);
                    cubicPathFill.cubicTo(
                            cpx, prev.getY() * phaseY,
                            cpx, cur.getY() * phaseY,
                            cur.getX(), cur.getY() * phaseY);
//                    cubicPathFill.lineTo(cur.getX(), fillMin);
//                    cubicPathFill.lineTo(prev.getX(), fillMin);
                } else {
                    cubicPath.moveTo(cur.getX(), cur.getY() * phaseY);
                    cubicPathFill.moveTo(cur.getX(), cur.getY() * phaseY);
                }
        ...
        if (dataSet.isDrawFilledEnabled()) {
            Log.d("drawHorizontalBezier", "inside draw fill");
            cubicFillPath.reset();
            cubicFillPath.addPath(cubicPathFill);
            // create a new path, this is bad for performance
            drawCubicFill(mBitmapCanvas, dataSet, cubicFillPath, trans, mXBounds, lastGradientPoint);
        }
    ...
    protected void drawCubicFill(Canvas c, ILineDataSet dataSet, Path spline, Transformer trans, XBounds bounds, float lastGradientPoint) {

        float fillMin = dataSet.getFillFormatter()
                .getFillLinePosition(dataSet, mChart);
        Log.d("drawCubicFill", " " + fillMin);
        Log.d("drawCubicFill", "lastGradientPoint " + lastGradientPoint);
        Log.d("drawCubicFill", "dataset " + dataSet.getEntryForIndex(bounds.min).getX());
        spline.lineTo(3, fillMin);
        spline.lineTo(2, fillMin);
        spline.lineTo(1, fillMin);
        spline.lineTo(0, fillMin);

        spline.lineTo(lastGradientPoint, fillMin);
        spline.lineTo(5, fillMin);
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            // let the spline start
            cubicPath.moveTo(cur.getX(), cur.getY() * phaseY);
            singlePointPath.moveTo(cur.getX(), cur.getY() * phaseY);
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                cubicPath.cubicTo(prev.getX() + prevDx,
...
//        mRenderPaint.setPathEffect(new CornerPathEffect(10F));
//        mRenderPaint.setAntiAlias(true);
       ...
       // mRenderPaint.setStrokeCap(Paint.Cap.ROUND);
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            if (e1 != null) {
                Log.d("linechart", "asdasasdsda" + e1.getX() + " " + e1.getY());
                int j = 0;
                for (int x = mXBounds.min; x <= mXBounds.range + mXBounds.min; x++) {
                    ...
                    if (e1 == null || e2 == null || e1.getY() == 0 || e2.getY() == 0) continue;
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                if (dataSet.isShowLastCircle()) {
                    if (entry.getY() != 0) {
                        circleIndex = j;
                    }
                } else if (dataSet.isShowMaxCircle()) {
                    if (entry.getY() != 0) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                if (e == null) break;
                Log.d("drawcircles", "inside circles 2:: " + e.getX() + " " + e.getY());
                ...
                if (circleIndex != -1) {
                    if (circleBitmap != null && circleIndex == j) {
                        Log.d("drawcircles", "print index");
...
//                Log.d("maxCircle", "jkhfkd " + set.getMaxOrLastCircleColor());
//                if (set.getMaxOrLastCircleColor() != -1) {
//                    Log.d("maxCircle", "jkhfkd " + set.getMaxOrLastCircleColor());
//                    mRenderPaint.setColor(set.getMaxOrLastCircleColor());
//                } else {
//                    Log.d("maxCircle", "why");
