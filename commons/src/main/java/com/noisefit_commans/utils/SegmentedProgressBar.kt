...
class SegmentedProgressBar : View, Runnable, ViewPager.OnPageChangeListener {
    ...
    private fun loadSegment(offset: Int, userAction: Boolean) {
        ...
        nextSegmentIndex = oldSegmentIndex + offset


        //when user remove call back
        if (oldSegmentIndex < 0 && offset < 0) {
            if (segmentCount == 2)
                nextSegmentIndex = 1
            else
                nextSegmentIndex = 7
