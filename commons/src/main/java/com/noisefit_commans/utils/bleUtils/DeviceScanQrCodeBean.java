...
public class DeviceScanQrCodeBean implements Parcelable {
    private static final String TAG = DeviceScanQrCodeBean.class.getSimpleName();
    ...
    public static final Creator<DeviceScanQrCodeBean> CREATOR = new Creator<>() {
        @Override
    ...
    public static class DeviceRadioBroadcastBean implements Parcelable {
        ...
        public static final Creator<DeviceRadioBroadcastBean> CREATOR = new Creator<>() {
            @Override
...
//        }
        ...
        public DeviceRadioBroadcastBean(String input_data) {
            Log.d(TAG, "DeviceRadioBroadcastBean = input_data = " + input_data);
            if (input_data != null && !input_data.equals("") && !input_data.equals("null")) {
                ...
                Log.d(TAG, "DeviceRadioBroadcastBean = my_data = " + bytes2HexString(my_data));
                Log.d(TAG, "DeviceRadioBroadcastBean = my_data len = " + my_data.length);
                setData(my_data);
                //旧协议
...
//                    setDeviceType(10026);
                    ...
                    int[] deviceParams = BinstrToIntArray(data[11]);
                    // 未绑定
                ...
                //新协议
                // D6360330000C7E75010007B40000D6360330000C
                if (data.length >= 20) {
                    ...
                    for (int i = 12; i < 20; i++) {
                        serialNumber.append(String.format(Locale.ENGLISH, "%02X", data[i]));
                        if (i != 19) {
                            serialNumber.append(":");
                        }
                    }
                    setSerialNumber(serialNumber.toString().toUpperCase(Locale.ROOT));
...
//                    "data=" + bytes2HexString(data) +
    ...
    public static int[] BinstrToIntArray(byte my_byte) {
        ...
        for (int i = byte_str.length() - 1; i >= 0; i--) {
            result[i + 8 - byte_str.length()] = Integer.parseInt(byte_str.substring(i, i + 1));
        }
        return result;
    }
